    ...
    <section className="py-12 md:py-20 w-full">
      <div className="section-container">
        <div className="text-center mb-10">
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-slate-800 mb-3">📈 Acompanhe o Progresso</h2>
          <p className="text-slate-500 text-sm md:text-base">Celebre cada conquista no desenvolvimento da atenção</p>
        </div>
        ...
        <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 mb-8">
          ...
          <div className="card">
            ...
            <div className="grid grid-cols-2 gap-3">
              {achievements.map((a) => (
                <div key={a.id} className={`p-3 rounded-xl text-center border-2 transition-all ${a.unlocked ? "border-indigo-200 bg-indigo-50" : "border-slate-100 bg-slate-50 opacity-60"}`}>
                  <div className="text-2xl mb-1">{a.emoji}</div>
                  <div className="font-semibold text-xs text-slate-700">{a.title}</div>
                  <div className="text-xs text-slate-400 mt-0.5">{a.desc}</div>
                  {a.unlocked && <div className="text-xs text-indigo-600 font-medium mt-1">✓ Desbloqueada</div>}
                </div>
              ))}
            </div>
          </div>
